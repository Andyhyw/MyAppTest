package com.example.app3.sp.sputils;
...
/**
 *
 * todo SharedPreferences是无法直接保存Object,List集合和Map集合的，需要先将它们转换为字符串，
 * todo 在保存到SharedPreferences中，取得时候再把对应的字符串转为Object,List集合或者Map集合。
 * todo 我这里的做法是利用Gson将Object,List或者Map转成Json保存到SharedPreferences中，
 * todo 取数据的时候再通过Gson将json数据转成Object,List集合或者Map集合。
 */
...
public class SharedPreferencesUtil {
     ...
     */
    public static void getInstance(Context context, String name) {//单例   饿汉  饱汉
        if (util == null) {
            ...
            switch (type) {
                ...
                default:
                    ...
                    if (!json.equals("") && json.length() > 0) {
                        result = gson.fromJson(json, defaultValue.getClass());//Gson  FastJson
                    } else {
                        result = defaultValue;
                    }
                    break;
